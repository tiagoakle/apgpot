...
%Jan 17 2013
%Test for proxpottoy like algorithm but the gradient before projection
%is that of psi

%x+ = prox{ th(z) + 0.5||x-t dPsi -z||^2_2}
...
% set defaults:
defaults = {...
    'accel',0;... %Use nesterov acceleration
...
for itn = 1:pars.max_iter
   ...
   if (mod(itn,pars.itn_print) == 0 || itn ==1)&&(pars.itn_print>0)
    ...
    if pars.accel > 0  %Acceleration step
        ...
        f  = nP^2+nD^2+nG^2;
        ps = 2*rho*log(f) - sum(log(x)) - sum(log(z));
